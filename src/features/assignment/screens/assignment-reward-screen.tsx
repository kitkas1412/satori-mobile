// Màn hình hiển thị phần thưởng sau khi hoàn thành bài trắc nghiệm.
// Hiển thị tuần tự theo queue: Streak → Level → từng Badge.
// Dữ liệu đọc từ Zustand store (đã được lưu bởi useQuizSubmit).
...
function StreakView({ data, theme }: { data: StreakNotification; theme: typeof Colors.light }) {
  ...
  return (
    <View className="flex-1 items-center justify-center px-8 gap-6">
      ...
      <View className="items-center gap-3">
        <Text className="font-heading text-2xl text-center" style={{ color: theme.text.primary }}>
          ...
          <View key={label} className="items-center gap-1">
            <View
              ...
              style={{
                width: 36,
                height: 36,
                backgroundColor: weekDays[i] ? theme.warning.default : theme.border.subtle,
              }}
            >
              {weekDays[i] && <Check size={20} color="white" strokeWidth={2.5} />}
            </View>
...
function LevelUpView({ data, theme }: { data: LevelUp; theme: typeof Colors.light }) {
  return (
    <View className="flex-1 items-center justify-center px-8 gap-6">
      ...
      {data.isLevelUp ? (
        <View className="items-center gap-4">
          ...
          <View className="flex-row items-center gap-3">
            <View className="rounded-full px-4 py-2" style={{ backgroundColor: theme.border.subtle }}>
              <Text className="font-heading text-lg" style={{ color: theme.text.secondary }}>
                Level {data.previousLevel}
              </Text>
            </View>
            <Text className="font-heading text-xl" style={{ color: theme.text.secondary }}>
              →
            </Text>
            <View className="rounded-full px-4 py-2" style={{ backgroundColor: theme.brand.primary }}>
              <Text className="font-heading text-lg" style={{ color: theme.brand.onPrimary }}>
                Level {data.newLevel}
              </Text>
            </View>
          </View>
      ...
      <Text className="font-heading text-3xl" style={{ color: theme.brand.primary }}>
        +{data.expEarned} EXP
      </Text>
...
function BadgeView({ data, theme }: { data: BadgeEarned; theme: typeof Colors.light }) {
  ...
  return (
    <View className="flex-1 items-center justify-center px-8 gap-4">
      ...
      <View className="items-center gap-2">
        <Text className="font-body text-sm text-center" style={{ color: theme.text.secondary }}>
          Huy hiệu mới nhận được
        </Text>
        <Text className="font-heading text-2xl text-center" style={{ color: theme.purple.default }}>
          {data.badgeName}
        </Text>
      </View>
...
export function AssignmentRewardScreen() {
  ...
  return (
    ...
    >
      {current.type === "streak" && <StreakView data={current.data} theme={theme} />}
      {current.type === "level" && <LevelUpView data={current.data} theme={theme} />}
      {current.type === "badge" && <BadgeView data={current.data} theme={theme} />}

      <View className="px-4" style={{ paddingBottom: Math.max(insets.bottom, 16) + 8 }}>
        <PrimaryButton text="Tiếp tục" variant="dark" onPress={handleNext} />
      </View>
    </View>
  );
}
